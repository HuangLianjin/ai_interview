...
- 演示地址：https://interview.huanglianjin.icu
- 演示环境可直接注册体验，验证码会显示在接口返回中。

---
## 📖 项目简介

职面 AI是一个利用大语言模型（LLM）和 LangGraph 状态机技术构建的综合求职辅助系统。它不仅能进行全真模拟面试，还能像专业的职业咨询师一样，通过多智能体协作（Multi-Agent）对简历进行深度诊断和定向优化，帮助求职者全方位提升竞争力。现已上线测试：[职面 AI](https://interview.huanglianjin.icu)
...
```mermaid
flowchart LR
    A[面试者] --> B[Next.js 前端 :3000]
    B -->|"/api"| C[Nginx]
    C --> B
    C -->|"/api/"| D[FastAPI 后端 :8000]
    D --> E[(PostgreSQL 16)]
    D --> F[DeepSeek / 通义千问 LLM]
    D --> G[Qwen ASR / Omni 语音]
```
...
- **结构化请求日志**：每个 HTTP 请求输出 JSON 日志，包含用户、路径、状态码、耗时。
- **用量统计**：`usage_logs` 记录请求量、平均耗时、错误数，并提供用户维度汇总接口。
- **数据闭环**：支持账号数据 JSON 导出、一键注销删除，面试/简历/画像数据可完全清理。
- **上下文预算**：多轮面试自动压缩历史问答，控制 Token 成本并保留最近关键上下文。
- **可解释评分**：每题输出亮点、不足、可落地的改进建议，报告逐题展示。
- **接口安全**：全局限流 + 验证码限流 + 登录失败锁定，防刷防爆破。
- **可观测安全**：运行 Trace、成本与评测池仅管理员可访问，普通用户只能提交自己的反馈与申诉。
- **后台闭环**：Redis 分布式限流、数据库任务 worker 与常驻告警服务随 Compose 一起启动。
- **自动化验证**：后端 API 冒烟测试 + 上下文单元测试 + Playwright E2E，CI 自动跑类型检查和构建。
...
访问 `http://localhost`，注册后即可登录（演示环境验证码会直接显示）。
...
如需商业授权，请通过 GitHub Issue 联系项目维护者。

...
import { PanelLeft, Bot, Loader2, Award, Plus, MessageCircle, FileText, ArrowDown, Square, Lightbulb, X, Mic, Volume2 } from "lucide-react";
...
export default function InterviewPage() {
  ...
  const [userAvatar, setUserAvatar] = useState("teal");
  ...
  const [activeMainTab, setActiveMainTab] = useState<ViewType>(() => {
    if (typeof window === 'undefined') return 'landing';
    const token = localStorage.getItem('ai_interview_token');
    const expires = Number(localStorage.getItem('ai_interview_token_expires') || 0);
    if (!token || !expires || Date.now() >= expires) return 'landing';
    const saved = localStorage.getItem('activeMainTab') as ViewType;
    return saved === 'interview' || saved === 'resume' ? saved : 'landing';
  });
  ...
  useEffect(() => {
    setIsMounted(true);
    if (isAuthenticated()) fetchSessions(undefined);
  }, [fetchSessions]);

  // 登录态检查：已登录时恢复用户信息；令牌过期时回首页并要求重新登录
  useEffect(() => {
    if (typeof window === 'undefined') return;
    const hadSession = !!localStorage.getItem('ai_interview_token') || !!localStorage.getItem('ai_interview_token_expires');
    if (isAuthenticated()) {
      setUserPhone(getPhone());
      setUserName(getNickname());
      setUserAvatar(getAvatar() || 'teal');
    } else if (hadSession) {
      setActiveMainTab('landing');
  ...
  // token 过期被 401 踢回时，自动回到首页并弹出登录框
  useEffect(() => {
    if (typeof window === 'undefined') return;
    if (new URLSearchParams(window.location.search).get('need_login') === '1') {
      setActiveMainTab('landing');
      setShowAuthDialog(true);
  ...
  // 语音识别与合成由服务端环境变量统一配置，前端无需单独填写
  const hasVoiceConfig = useMemo(() => true, []);
  ...
  const handleGetHint = async () => {
    ...
    try {
      ...
      const response = await fetch(
        `${process.env.NEXT_PUBLIC_API_URL || (process.env.NODE_ENV === 'development' ? 'http://localhost:8000' : '')}/api/chat/hint/${threadId}/${questionIndex}`,
        { headers: authHeaders() }
      );
  ...
  const handleSpeakQuestion = async (text?: string) => {
    const content = text || [...messages].reverse().find(m => m.role === 'assistant')?.content;
    if (!content) return;
    try {
      const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL || (process.env.NODE_ENV === 'development' ? 'http://localhost:8000' : '')}/api/voice/tts?text=${encodeURIComponent(content.slice(0, 500))}`, { headers: authHeaders() });
  ...
  const handleSwitchToVoice = async () => {
    // 开箱即用：浏览器语音识别输入 + Edge TTS 朗读，不需要额外配置语音模型
    toggleListening();
    const lastAssistant = [...messages].reverse().find(m => m.role === 'assistant');
    if (lastAssistant) await handleSpeakQuestion(lastAssistant.content);
    toast.success('语音输入已开启，可直接说话回答');
  };
  ...
  // 打开旧会话：已完成定位到第一题（顶部），进行中定位到最新消息（底部）
  const prevSessionIdRef = useRef<string | null>(null);
  const scrollPositionOnSelectRef = useRef<'top' | 'bottom' | null>(null);
  useEffect(() => {
    const sessionId = currentSession?.session_id || null;
    if (sessionId && sessionId !== prevSessionIdRef.current) {
      prevSessionIdRef.current = sessionId;
      scrollPositionOnSelectRef.current = currentSession?.metadata.status === 'completed' ? 'top' : 'bottom';
    }
  }, [currentSession?.session_id]);

  // 自动滚动效果
  useEffect(() => {
    const position = scrollPositionOnSelectRef.current;
    if (position) {
      scrollPositionOnSelectRef.current = null;
      if (position === 'top') {
  ...
  const handleAuthSuccess = (phone: string, nickname = "", avatar = "teal") => {
    setUserPhone(phone);
    setUserName(nickname || "");
    setUserAvatar(avatar || "teal");
      ...
      <main className="flex-1 flex flex-col min-h-0 relative bg-white overflow-hidden">
        ...
        <div className="border-b border-gray-100 bg-white/80 backdrop-blur-sm sticky top-0 z-20">
          <div className="mx-auto px-6 py-2 flex items-center justify-between">
            ...
            <div className="flex items-center gap-2">
              {userPhone && <span className="text-xs text-gray-500">{userPhone}</span>}
        ...
        {activeMainTab === "resume" ? (
                    ...
                    <div className="flex items-center justify-between text-sm">
                      ...
                      {interviewProgress.current < interviewProgress.total && (
                        ...
                        <Button
                          variant="ghost"
                          size="sm"
                          className="text-teal-700 hover:text-teal-800 hover:bg-teal-50 gap-1.5 h-7 px-2"
                                  ...
                                  <Button
                                    onClick={async () => {
                                      try {
                                        ...
                                        // 1. 创建下一轮会话
                                        const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL || (process.env.NODE_ENV === 'development' ? 'http://localhost:8000' : '')}/api/sessions/${currentSession.session_id}/next-round`, {
                                          method: 'POST',
                                          headers: {
          ...
          'Authorization': `Bearer ${getToken() || ''}`,
                                        ...
                                        const startResponse = await fetch(`${process.env.NEXT_PUBLIC_API_URL || (process.env.NODE_ENV === 'development' ? 'http://localhost:8000' : '')}/api/chat/start`, {
                                          method: 'POST',
                                          headers: {
                                            'Content-Type': 'application/json',
                                            'X-User-ID': getUserId(),
        ...
        <SettingsDialog open={showSettingsDialog} onOpenChange={setShowSettingsDialog} />
        <AuthDialog open={showAuthDialog} onOpenChange={setShowAuthDialog} onSuccess={handleAuthSuccess} />
        <ProfileDialog open={showProfileDialog} onOpenChange={setShowProfileDialog} currentNickname={userName} currentAvatar={userAvatar} onSave={handleSaveProfile} />

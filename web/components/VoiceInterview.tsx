...
export function VoiceInterview({ sessionId, onEnd }: VoiceInterviewProps) {
    ...
    useEffect(() => {
        const initSession = async () => {
            ...
            try {
                ...
                if (!apiConfig) {
                    toast.error('语音功能暂未开启，请使用文字面试');
                    onEnd();
                    return;
                }

                const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL || (process.env.NODE_ENV === 'development' ? 'http://localhost:8000' : '')}/api/voice/start`, {
                    method: 'POST',
                    headers: {
    ...
    async function sendToOmni(
        ...
        try {
            ...
            if (audioBlob) {
                ...
                // 使用 Qwen3-ASR-Flash 转写，成功后以文本形式发给大模型
                try {
                    const sttText = await transcribeAudio(audioBlob);
                    if (sttText && sttText.trim()) {
                        textMessage = sttText.trim();
                        console.log(`[VoiceInterview] ASR 转写成功: ${textMessage}`);
                    }
                } catch (sttError) {
                    console.warn('[VoiceInterview] ASR 转写失败，回退到浏览器识别:', sttError);
            ...
            const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL || (process.env.NODE_ENV === 'development' ? 'http://localhost:8000' : '')}/api/voice/chat`, {
                method: 'POST',
                ...
                body: JSON.stringify({
                    ...
                    audio: null,
                    message: textMessage,  // Qwen3-ASR-Flash 转写后的文本
    ...
    // 调用后端 Qwen3-ASR-Flash 将录音转写为中文文本
    async function transcribeAudio(audioBlob: Blob): Promise<string> {
        const formData = new FormData();
        formData.append('file', audioBlob, 'speech.wav');

        const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL || (process.env.NODE_ENV === 'development' ? 'http://localhost:8000' : '')}/api/voice/stt`, {
            method: 'POST',
            headers: {
                'Authorization': `Bearer ${getToken() || ''}`,
            },
        ...
        if (!response.ok) {
            const error = await response.json().catch(() => ({ message: `语音转写失败 (${response.status})` }));
            throw new Error(error.detail || error.message || '语音转写失败');
        }

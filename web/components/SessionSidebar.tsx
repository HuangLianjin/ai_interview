...
const AVATAR_COLOR_CLASS: Record<string, string> = { teal: 'bg-teal-500', blue: 'bg-blue-500', purple: 'bg-purple-500', orange: 'bg-orange-500', green: 'bg-green-500', pink: 'bg-pink-500' };
                        ...
                        <div className="p-4 border-t border-gray-200 space-y-2">
                            ...
                            <div className="flex items-center gap-3 px-2 py-2">
                                {userAvatar.startsWith('/' ) || userAvatar.startsWith('http') ? (
                                    <img src={userAvatar} alt="头像" className="h-8 w-8 rounded-full object-cover border border-gray-200" />
                                ) : (
                                <div className={cn("h-8 w-8 rounded-full flex items-center justify-center text-white text-sm font-bold", AVATAR_COLOR_CLASS[userAvatar] || "bg-teal-500")}>
                                    {(userName || userPhone || "用").slice(0, 1).toUpperCase()}
                                </div>
                                )}
                                <div className="flex-1 min-w-0">
                                    <p className="text-sm font-medium text-gray-900 truncate">{userName || "面试候选人"}</p>
                                    <p className="text-xs text-gray-500 truncate">{userPhone || "已登录"}</p>
                                </div>
                            </div>
                        </div>
                    </motion.aside>

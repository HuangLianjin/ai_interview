...
async def init_database():
    ...
    try:
        ...
        ''')
        logger.info("✓ users 表已创建/验证")

        await conn.execute("ALTER TABLE users ADD COLUMN IF NOT EXISTS nickname VARCHAR(50) DEFAULT ''")
        await conn.execute("ALTER TABLE users ADD COLUMN IF NOT EXISTS avatar TEXT DEFAULT 'teal'")
        ...
        ''')
        logger.info("✓ login_attempts 表已创建/验证")
        ...
        ''')
        await conn.execute('''
            CREATE INDEX IF NOT EXISTS idx_answer_scores_session
            ON answer_scores(session_id, question_index)
        ''')
        logger.info("✓ answer_scores 表已创建/验证")
        ...
        ''')
        logger.info("✓ knowledge_chunks 表已创建/验证")
        await conn.execute('''
        ...
        ''')
        logger.info("✓ usage_logs 表已创建/验证")

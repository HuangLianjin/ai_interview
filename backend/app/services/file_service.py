...
def decode_filename(filename: str) -> str:
    """解码 multipart 上传文件名的 RFC 2047 编码（如 =?utf-8?B?...?=）。"""
    if not filename:
        return filename
...
class FileService:
    ...
    async def process_fastapi_file(self, upload_file: UploadFile) -> str:
        ...
        try:
            # 浏览器/客户端可能对非 ASCII 文件名做 RFC 2047 编码，先还原
            decoded_filename = decode_filename(upload_file.filename or "")

            # 1. 验证文件类型
            if not self._validate_file_type(decoded_filename):
                raise UnsupportedFileTypeError(
                    f"不支持的文件类型: {decoded_filename}。"
            ...
            # 3. 创建临时文件进行处理
            file_ext = os.path.splitext(decoded_filename)[1]

...
async def event_generator(graph, inputs, config, thread_id: str, user_message: str) -> AsyncGenerator[str, None]:
    ...
    try:
        ...
        # 内部节点返回但未流式输出的 AI 消息（如追问）补发给前端
        if not ai_response_content and fallback_ai_content:
            ai_response_content = fallback_ai_content
            fb = ChatStreamResponse(type="token", content=fallback_ai_content)
            yield f"data: {fb.model_dump_json()}\n\n"
        ...
        # 单题作答评分（基于本轮刚回答的问题）
        answered_idx = inputs.get("current_question_index", 0)
        plan = inputs.get("interview_plan") or []
        if 0 <= answered_idx < len(plan):
